...
static void SimulatedExistingApp_BringUpNetif(void* context)
{
    ...
    (void) netif_add(&s_interface, &ipAddress, &netmask, &gateway, NULL, EthernetIf_Init, tcpip_input);
...
bool SimulatedExistingApp_Start(void)
{
    ...
    /* lwIP: bring the netif up on the tcpip thread, then let smsc9220_init's
     * settle complete before anything relies on the link. */
    (void) tcpip_callback(SimulatedExistingApp_BringUpNetif, NULL);
